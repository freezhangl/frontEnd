...
/**
 * timestamp 当前时间的时间戳/1000,cutoffTime 指定日期时间戳
 * return  时间倒计时
 */
function cutoffTime(timestamp,cutoffTime){        //截止时间倒计时
    var leftTime = cutoffTime - timestamp; //计算剩余的毫秒数
    ...
    if(cutoffTime >= timestamp1){
    $("#timer").text(days+"天" + hours+"小时" + minutes+"分"+seconds+"秒");
    setTimeout(function () {
        cutoffTime(timestamp1, cutoffTime);
    }, 1000)
    } else {
        $("#timer").text('活动结束');
    }
...
/**
 * 获取URL传入的参数,并转换为json对象
 * @param url  location.search
 * @returns {string} 返回值是json对象
 */
...
function getFirstElement(element) {
    if (element.firstElementChild) {
        return element.firstElementChi
        ld;
    } else {
...
function setCookie(name, value, time) { //设置cookie1
    var strsec = getsec(time);
    var exp = new Date();
    exp.setTime(exp.getTime() + strsec * 1);
    document.cookie = name + "=" + encodeURIComponent (value) + ";expires=" + exp.toGMTString();
}
function getsec(str) { //设置cookie2 return设置过期时间的毫秒
