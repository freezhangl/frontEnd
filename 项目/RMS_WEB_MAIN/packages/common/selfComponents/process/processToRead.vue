...
export default {
  ...
  props: {
    ...
    // processTemplateCode: { type: Number, default: 0 },
  ...
  methods: {
    ...
    submitApproveForm () {
      ...
      let selectDepartmentIdList = []
      this.selectDepartment.forEach(item => {
        selectDepartmentIdList.push(item.id)
      })
      const params = {}
      params.recipients = selectDepartmentIdList.join(',')
      params.title = this.title
      // params.title = '测试立项审批通过终止流程'
      params.workItemId = this.workItemId
      // params.workItemId = 2180
      API.sendTaskNotificationToPerson(params)
        .then(res => {
          this.$message({
            message: '送阅成功！',
            type: 'success',
    ...
    treeCheckChange (node, checked) {
      ...
      //     }
      //   })
      //   this.selectDepartment = this.selectDepartment.concat(checkedList)
      //   this.selectShowDepartment = this.selectDepartment
      // } else {
      //   this.selectDepartment = []
      //   this.selectShowDepartment = []
    ...
    async loadNode (node, resolve) {
      ...
      if (node.level >= 1) {
        ...
        /* for (let i = 0; i < this.treeData.length; i++) {
          org = this.getOrgObj(this.treeData[i], node.data.id)
          if (org) break
        }
        if (org) {
          //将节点数据拼接到树形数据上
          this.$set(org, 'children', [])
          this.$set(org, 'children', nodeData.children)
        } */
        // console.log('nodeData', nodeData, nodeData.children)
