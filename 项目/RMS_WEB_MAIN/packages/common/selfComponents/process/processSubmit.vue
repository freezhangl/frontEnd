  ...
  <div class="processSubmit processWraper">
    <el-dialog
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="下一环节：" prop="nextLink">
              <el-radio-group v-if="approvalForm.approvalState==='0' || approvalForm.approvalState==='2'" v-model="approvalForm.nextLink">
                <el-radio
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button v-if="isFinishAndNotification"  type="primary" @click="submitApproveForm">下一步</el-button>
        <el-button v-else type="primary" @click="submitApproveForm">提交</el-button>
      </span>
    </el-dialog>
    <el-dialog
      v-dialogDrag
      ...
      <div class="confirm" style="margin-bottom: 20px;" v-if="isPeople || isOrg || isPeopleOrg">
        <span v-show="(isPeople || isPeopleOrg) && finishFlag==1 ">人员：</span>
    ...
    <processExperts
      ...
      :is-to-read="isFinishAndNotification"
      @subSelectPeople="expertChoosen"
      @subToReadPeople="toReadPeopleChoosen"
...
export default {
  ...
  data () {
    return {
      ...
      toReadPeoleList: [], //送阅知选中人员集合
      modalTitle: '' //送阅知组件和专家选择弹框标题
    }
  },
  watch: {
    filterText (val) {
    ...
    'approvalForm.approvalState' (newVal, oldVal) {
      if (newVal === '0') {
        ...
        this.approvalStateShow = '会签'
      }
    },
    'approvalForm.nextLink' (newVal, oldVal) {
      if (this.approvalForm.approvalState === '0' || this.approvalForm.approvalState === '2') {
  ...
  methods: {
    ...
    //送阅知人员选择接收
    toReadPeopleChoosen (toReadPeoleList) {
      this.toReadPeoleList = toReadPeoleList
      this.confirmDialogVisible = true
    },
    //提交送阅知
    subToReadForm (obj) {
      let toReadPeoleIdList = []
      this.toReadPeoleList.forEach(item => {
        toReadPeoleIdList.push(item.id)
      })
      const params = {}
      params.recipients = toReadPeoleIdList.join(',')
      params.title = this.title
      // params.title = '测试立项审批通过终止流程'
      params.workItemId = this.workItemId
      // params.workItemId = 2180
      API2.sendTaskNotificationToPerson(params)
        .then(res => {
          this.$message({
            message: '送阅成功！',
            type: 'success',
          ...
          this.closeProcessExperts(false)
    ...
    getNextActivities (val) {
      ...
      API.getNextActivitiesAndNotification(params)
        .then(res => {
    ...
    getBackButton () {
      ...
      API.getBackButton(params)
        .then(res => {
          ...
          this.approvalForm.nextLink = ''
          this.isFinishAndNotification = false
        })
        .catch(e => {
          if (e.selfError) return
          if (e.message) {
    ...
    changeNextLink (val) {
      ...
      if (val.activityDefId === 'finishNotificationActivity') {
        this.isFinishAndNotification = true
      } else {
        this.isFinishAndNotification = false
      }
      // 如果是结束
    ...
    getNextPeople (activityDefId) {
      let params = {
        processTemplateCode: this.processTemplateCode, // 流程模板编码
        // processTemplateCode: 'groupApplicationModel', // 流程模板编码
        activityDefId: activityDefId, // 当前节点ID
        // activityDefId: 'subflowActivity', // 当前节点ID
        businessSerialNumber: this.businessSerialNumber //业务流水号
        // businessSerialNumber: 'Q20200000802407' //业务流水号
      }
    ...
    submitApproveForm () {
      ...
      } else if (this.approvalForm.approvalState === '2') { //其他
        if (this.isOrg === true) { //部门会签（组织树）
          flag = this.validTree()
        } else if (this.isPeopleOrg === true) { //部门会签（组织+人员）
          flag = this.validTreeOrg()
        } else { //专家会签
          let peopleAdd = {
            peopleChoosen: [
      ...
      this.$nextTick(() => {
        this.$refs['approvalForm'].validate(valid => {
          if (valid && flag) {
            if (this.isFinishAndNotification) {
              this.openNotificationMoal()
            } else {
              this.confirmDialogVisible = true
            }
          }
    ...
    submitConfirmDialog () {
      ...
      if (obj.nextActivityDefId === 'finishNotificationActivity') { //结束并发送阅知
        this.subToReadForm(obj)
      } else {
        this.$emit('confirmSubmit', obj)
      }
