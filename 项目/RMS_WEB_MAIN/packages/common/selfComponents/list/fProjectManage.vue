...
项目经理组件
<component
  :is="fProjectManage" // fProjectManage项目经理
        ...
        <div>
          ...
          <el-select
            ...
            <el-option
              ...
              :value="item.userId">
              <el-tooltip :content="item.chineseName + '/' + item.userId" :disabled="!showToolTip(item.chineseName + '/' + item.userId, selectWidth)">
                <span>{{ item.chineseName + '/' + item.userId }}</span>
              </el-tooltip>
            </el-option>
          </el-select>
        </div>
...
export default {
  data () {
    return {
      value: this.data ? JSON.parse(this.data) : [],
      options: [],
      isTrue: true, //目前只有多选的情况
      arrowNum: 0,
      arrowPath: 'left',
  ...
  mounted () {
    if (this.status !== 'result') {
      API.getProjectManage({ orgCode: this.$store.getters['USERENTITY/curUserOrg'].orgCode })
        .then(data => {
          this.options = data
  ...
  methods: {
    ...
    update (v) {
      ...
      if (this.isTrue) {
        ...
        this.options.forEach((item, index) => {
          if (item.userId === v) {
            stringA = item.chineseName
          }
        })
        this.props.previewName = stringA
      }
      v = this.isTrue ? JSON.stringify(v) : v
