...
项目总监组件
<component
  :is="fProjectDirector" // fProjectDirector项目总监
        ...
        <div>
          ...
          <el-select
            ...
            <el-option
              ...
              :value="item.userId">
              <el-tooltip :content="item.chineseName+'/'+item.email" :disabled="!showToolTip(item.chineseName+'/'+item.email, selectWidth)">
                <span>{{item.chineseName}}/{{item.email}}</span>
              </el-tooltip>
            </el-option>
          </el-select>
        </div>
...
export default {
      ...
      this.options.forEach(item => {
        this.value.forEach(item1 => {
          if (item.userId === item1) {
            previewName.push(item.chineseName)
          }
        })
      })
  ...
  mounted () {
    if (this.status !== 'result') {
      API.getProjectDirectors({ orgCode: this.userInfo.userAuth.orgCode })
        .then(data => {
          this.options = data
      ...
      return function (codeArr, objArr) {
        ...
        if (arr && objArr) {
          for (let i = 0; i < objArr.length; i++) {
            let num = codeArr.indexOf(objArr[i].userId)
            if (num !== -1) {
              arr.splice(num, 1, objArr[i].chineseName)
            }
          }
          return arr.toString()
        } else {
  ...
  methods: {
    ...
    arrowChangeClass () {
      setTimeout(() => {
        let innerWidths = this.$refs[`advancedSelect${this.props.id}`].$el.querySelectorAll('.el-select__tags .el-tag')
