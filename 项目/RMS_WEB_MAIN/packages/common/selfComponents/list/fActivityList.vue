...
研发活动清单组件
<component
  :is="fActivityList" // fActivityList研发活动清单
  ...
  <div :class="props.isClass">
    ...
    >
      ...
      <el-tooltip
        ...
        effect="dark"
        v-if="status !== 'result'"
        :disabled="!showToolTip(contentTips, selectWidth)"
        :content="contentTips"
        placement="top">
        <el-select
          :ref="`select${props.id}`"
          ...
          <el-option
            ...
            <el-tooltip
              ...
              :disabled="!showToolTip(item.devName, selectWidth)">
...
export default {
  ...
  mounted () {
    API.getActivityList({ devLevel: 3, status: 0 })
      .then(data => {
        // 数组去重
        var obj = {}
        ...
        if (this.data) {
          this.list.forEach((item, index) => {
            if (item.devCode === this.data) {
              this.props.previewName = item.devName
              this.contentTips = item.devName
            }
          })
        }
        // 下拉选择溢出设置宽度
  ...
  methods: {
    ...
    getPreviewName (name) {
      if (name && this.list && this.list.length > 0) {
        this.list.forEach((item, index) => {
          if (item.devCode === name) {
            this.props.previewName = item.devName
            this.contentTips = item.devName
          }
        })
