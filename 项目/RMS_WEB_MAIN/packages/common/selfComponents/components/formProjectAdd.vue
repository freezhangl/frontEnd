            ...
            data.blockDtos.forEach((item, index) => {
              item.fieldList.forEach((item1, field) => {
                this.formRulesIn.forEach((itemIn, indexIn) => {
                  // 字符串和数组两种判断形式
                  if ((typeof (itemIn.value) === 'string' && itemIn.key === item1.fieldKey && itemIn.value === item1.defaultValue) || (itemIn.value instanceof Array && itemIn.key === item1.fieldKey && itemIn.value.includes(item1.defaultValue))) {
                    itemIn.attrs.forEach((itemIn1, indexIn1) => {
                      keys.push({ key: itemIn1.key, isRequire: itemIn1.attrs.isRequire, isReadonly: itemIn1.attrs.isReadonly, isVisible: itemIn1.attrs.isVisible })
                    })
                  }
                })
              })
