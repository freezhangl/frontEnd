      ...
      <el-form
        ...
        <div>
          ...
          <el-form-item label="承担单位:" prop="undertakeCompany" :label-width="searchMoreFlag=='0'?'90px':'130px'">
            ...
            <el-tooltip
              v-if="searchForm.undertakeCompanyType"
              class="item"
              effect="dark"
              ...
              <el-select
                ...
                class="demandItemTwo"
                placeholder="全部"
                v-model="searchForm.undertakeCompanyType"
              >
                <el-option
                  v-for="item in undertakingUnitTypeOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
    ...
    <div class="main-warpper">
      ...
      <el-table v-loading="loading" :data="tableDataOut.list" style="overflow: auto;">
        ...
        <el-table-column
          ...
          <template slot-scope="scope"><div @click="handleClick(scope.row)" class="jump-detail">
      ...
      <div v-if="tableDataOut.total>0">注：权重=需求单位给承担单位的总预算/需求单位总预算</div>
      <div v-if="tableDataOut.total>0">实际得分=结项评分*权重</div>
    </div>
    <el-progress
...
export default {
  ...
  data () {
    return {
      ...
      undertakingUnitOptions: [], //承担单位 options
      undertakingUnitTypeOptions: [{ value: 1, label: '牵头' }, { value: 0, label: '配合' }], //承担单位类型 options
  ...
  computed: {
    ...
    changeUndertakeCompanyType () {
      let val = ''
      if (this.searchForm.undertakeCompanyType) {
        this.undertakingUnitTypeOptions.forEach(item => {
          if (this.searchForm.undertakeCompanyType === item.value) {
            val = item.label
          }
        })
      }
  ...
  methods: {
    ...
    download () {
      let param = {
        ...
        undertakeCompanyType: this.searchForm.undertakeCompanyType,
      ...
      if (param.demandCompanyType !== '' && param.demandCompany === '') {
      ...
      } else if (param.undertakeCompanyType !== '' && param.undertakeCompany === '') {
        this.$message({
          message: '请选择承担单位',
          type: 'error',
          showClose: true
        ...
        '&undertakeCompany=' + param.undertakeCompany + '&undertakeCompanyType=' + param.undertakeCompanyType + '&projectClass=' + param.projectClass +
        ...
        axios.request({
          url: this.baseUrl + '/rms/project/export/v1/allScoreUndertakeExport' + url,
          headers: {
            'Authorization': this.Authorization,
            'pragma': 'no-cache',
            'cache-control': 'no-cache'
    ...
    reset () {
      ...
      this.searchForm.undertakeCompanyType = ''
    ...
    getList (pageNum, pageSize) {
      ...
      if (param.demandCompanyType !== '' && param.demandCompany === '') {
        ...
        API.queryFinishScoreUndertake(param)
          .then(res => {
            if (res.total > 0) {
              this.tableDataOut.list = res.scoreUndertake
            } else if (res.scoreUndertake.length === 0) {
              this.tableDataOut.list = []
              this.tableDataOut.total = 0
            }
