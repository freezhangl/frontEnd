...
export default {
  ...
  methods: {
          ...
          if (valid && virtual && physical && storage) {
            let virtualList = virtual.map(item => {
              return {
                ...
                resourceEnvironmentValue: this.getResourceEnvironmentValue(item.resourceEnvironment),
    ...
    // 根据环境编码获取对应的值
    getResourceEnvironmentValue (code) {
      let value = null
      this.virtualEnvOptions.forEach(item => {
        if (code === item.valueCode) {
          value = item.name
        }
      })
      return value
