...
export default {
  ...
  watch: {
    ...
    receiveData (value) {
      if (this.projectAttribute && this.projectAttribute.schemeId) {
        ...
        this.obj = {
          ...
          schemeId: this.projectAttribute.schemeId,
          schemeJson: this.projectAttribute.extendedAttributes
        }
        ...
        blockDtos.forEach((item, index) => {
          item.fieldList.forEach((item1, index1) => {
            if (item1.fieldKey === 'accountingSubject') { //核算主体 只有草稿状态可编辑
              this.$refs.formProjectEdit.list.blockDtos[index].fieldList[index1].isReadonly = (status && status === '0') ? '0' : '1'
            }
            if (item1.fieldKey === 'additionalDeduction') { //是否加计扣除 只有草稿可编辑
              this.$refs.formProjectEdit.list.blockDtos[index].fieldList[index1].isReadonly = (status && status === '0') ? '0' : '1'
            }
          })
        })
      ...
      if (key === 'internalProjectCode' && value) {
        ...
        API.queryEffectiveProjectBaseInfoUrl({
          ...
          .then(res => {
            ...
            this.$refs.formProjectEdit.changeKeyValue(arr)
            this.isThen = '1'
          })
          .catch(err => {
