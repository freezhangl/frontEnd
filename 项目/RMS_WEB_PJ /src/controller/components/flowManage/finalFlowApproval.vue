<template>
  <div class="final-detail">
    ...
    <div class="main-container" v-if="showDetail">
      ...
      <div class="main-content">
        ...
        <div id="approved-con">
          <approval-opinion v-if="hasApproveOpinion" :business-opinions-array="approvalOpinionList"></approval-opinion>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      // projectFinTime: null, // 结项审批时间-TODO需要替换
      extendedAttributes: {
        remark: '', //备注信息
        projectFinTime: null // 结项审批时间-TODO需要替换
      },
  ...
  methods: {
      ...
      API.queryFinalFlowDetail(param)
        .then(res => {
          if (res) {
            ...
            // this.projectFinTime = this.formatDate(res.extendedAttribute.projectFinTime)
            // this.projectFinTime = this.formatDate(JSON.parse(res.projectApprove.extendedAttributes).projectFinTime)
            this.extendedAttributes.projectFinTime = res.projectApprove.extendedAttributes ? this.formatDate(JSON.parse(res.projectApprove.extendedAttributes).projectFinTime) : this.formatDate(res.projectBaseInfo.actualFinishTime)
            this.extendedAttributes.remark = JSON.parse(res.projectApprove.extendedAttributes).remark ? JSON.parse(res.projectApprove.extendedAttributes).remark : ''
    ...
    getProSubmitInfo (info) {
      ...
      this.processSubmitForm.businessType = 'BT-FINISH'
