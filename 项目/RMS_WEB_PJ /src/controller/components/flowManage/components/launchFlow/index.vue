<template>
  ...
  <div v-else style="text-align:center;color:#333333;">暂无立项流程信息</div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    // 打开立项流程编辑页面
    openEditPage () {
      // this.$router.push('/launchFlow/launchFlowEdit')
      this.$router.push({
    ...
    getLaunchInfo () {
      ...
      let params = {
        projectCode: this.projectInfo.projectCode,
        businessType: this.projectInfo.documentType //立项类型:枚举值
      }
      API.queryProjectApprove(params)
        .then(res => {
          ...
          if (res) {
            ...
            if (this.approveNo && res.historyFlag === '0') {
              ...
              switch (this.launchInfoForm.approveState) {
                case '1':
                  this.launchInfoForm.approveState = '草稿'
                  // this.launchInfoForm.flowName = this.processInstName + '草稿'
                  break
                case '2':
                  this.launchInfoForm.approveState = '审批中'
                  break
          ...
          // 判断发起立项流程的按钮是否显示:先判断用户是否具有权限-->是否存在草稿
          let falg = true
