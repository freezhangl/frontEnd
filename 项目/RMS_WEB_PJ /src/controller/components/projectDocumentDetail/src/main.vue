      ...
      // 使用组件props传递请求参数
        ...
        API.queryAttachmentType(params)
          .then(res => {
            ...
            // 过滤为空的记录--避免联系索引删除引发问题，使用for循环
            for (var i = this.projectDocList.length - 1; i > -1; i--) {
              if (this.projectDocList[i].fileName === '' || this.projectDocList[i].fileCode === null) {
                this.projectDocList.splice(i, 1)
              }
            }
          })
          .catch(e => {
