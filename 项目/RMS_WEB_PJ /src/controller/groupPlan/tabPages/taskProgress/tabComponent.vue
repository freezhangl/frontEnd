      ...
      <div class="add-progress" v-if="isEditable">
        <project-panel :title="'添加进展'">
          <div class="content">
            ...
            <div class="zhu">注: 每个单位至少每季度填写一次</div>
            <div class="add-btn">
          ...
          <template v-if="total !== 0">
            <div v-for="(item, index) in historyProcessList" :key="index" class="text-item">
              ...
              <div class="history-content">
                <span>进展说明：</span>
                <pre style="margin-top:0px;font-family: PingFangSC-Regular;font-size:14px;color:#333;display:block;width:95%">{{item.remarks}}</pre>
              </div>
            </div>
          </template>
...
export default {
  ...
  methods: {
    ...
    taskProcessList (obj) {
      ...
      let params = {}
      if (this.$route.query.projectCategory === 'd' && this.$route.query.developPackage === 'IS_RD_0') {
        params = {
          companyCode: this.companyCode, //公司编码
          // userCode: this.userInfo.userAuth.uid, //用户账号
      ...
      API.projectProgressList(params)
        .then(res => {
          this.isEditable = res.canAddProgress
          this.total = res.projectProgress.total
          if (!res.projectProgress.total) {
            // this.$message({
            //   message: '暂无数据!',
            //   type: 'error'
            // })
          ...
          if (res.projectProgress.list.length) {
            if (obj && obj.isToDown) {
              this.historyProcessList = this.historyProcessList.concat(res.projectProgress.list)
            } else {
              this.historyProcessList = res.projectProgress.list
            }
            //时间戳
          ...
          if (!res.projectProgress.isLastPage) this.isMoreLoad = true
        })
        .catch(() => {
        })
    },
    ...
    addTaskProcess () {
      let reg = /^\s+$/g
      if (this.textarea === '' || reg.test(this.textarea) === true) {
        // this.errorMsg = '进展说明不能为空'
        this.$message({
          type: 'error',
          message: '进展说明不能为空或纯空格输入!'
        })
        this.textarea = ''
        return false
      } else {
