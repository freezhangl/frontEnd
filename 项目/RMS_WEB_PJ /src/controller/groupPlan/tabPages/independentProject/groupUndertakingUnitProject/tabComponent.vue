    ...
    <div class="search-content">
      <el-form
        ...
        <div>
          ...
          <el-form-item label="承担部门:" prop="undertakeCompany" :label-width="searchMoreFlag=='0'?'90px':'130px'">
            <el-tooltip
              class="item"
              effect="dark"
              ...
              <div v-popover:popover1 class="showBox">
                ...
                <div v-else>
                  <div v-if="nameList.length>2" class="showNameBox">
                    <p v-for="(item,index) in nameList.slice(0,2)" :key="index">{{item}}</p>
                    <p>+{{nameList.length-2}}</p>
                  </div>
            ...
            >
              ...
              <div class="tree-con">
                <el-scrollbar>
                  <el-tree
                    class="trees custom-single-checkbox"
            ...
            <el-tooltip
              v-if="bearSearchForm.undertakeCompanyType"
              class="item"
              effect="dark"
              ...
              <el-select
                ...
                class="demandItemTwo"
                placeholder="全部"
                v-model="bearSearchForm.undertakeCompanyType"
                @change="changeInfo"
              >
                <el-option
          ...
          <el-form-item label="项目编码:" v-if="searchMoreFlag==='1'">
            <el-input
              v-model.trim="bearSearchForm.projectCode"
              :clearable="true"
              placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item label="项目名称:" v-if="searchMoreFlag==='1'">
            <el-input
              v-model.trim="bearSearchForm.projectName"
              :clearable="true"
              placeholder="请输入"></el-input>
          </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      bearBearUnitOptions: [], //承担部门 options
  ...
  computed: {
    ...
    changeUndertakeCompanyType () {
      let val = ''
      if (this.bearSearchForm.undertakeCompanyType) {
        this.bearUnitTypeOptions.forEach(item => {
          if (this.bearSearchForm.undertakeCompanyType === item.value) {
            val = item.label
          }
        })
      }
  ...
  methods: {
    ...
    exportFile () {
      ...
      let param = {
        ...
        undertakeCompanyType: this.bearSearchForm.undertakeCompanyType,
      ...
      let url = '?request=unit&projectYear=' + param.projectYear + '&demandCompany=' + param.demandCompany +
    ...
    changeValue (data, checked, node) {
      if (!data.disabled) {
        if (checked === true) {
          ...
          this.bearSearchForm.undertakeCompany = data.orgCode
        } else {
          if (this.checkedId === data.orgCode) {
            this.nameList = []
    ...
    getSearchData () {
      //获取属性信息的字段
      let data = { companyCode: this.userInfo.userAuth.companyCode, isAll: false, projectCategory: 'PC-UNDERTAKE-UNIT', businessObject: 'BO-OVERALL' }
    ...
    reset () {
      ...
      this.bearSearchForm.demandCompany = ''
      this.bearSearchForm.undertakeCompany = ''
      this.bearSearchForm.undertakeCompanyType = ''
    ...
    getList (pageNum, pageSize) {
      ...
      API.getIndependentUndertakeSearch(param)
        .then(res => {
          if (res.total > 0) {
