      ...
      <div class="main-content">
        <div v-if="scoreBody && Object.keys(scoreBody).length">
          <scoreEvaluateEdit
            ...
            :budget-data="budgetData"></scoreEvaluateEdit>
        </div>
        ...
        <!--  TODO 引入对应组件即可 -->
        <div id="basic-info">
          <basicInfo :receive-data="projectBaseInfo" :edit="true"></basicInfo>
        </div>
        ...
        <div id="approved-con" v-if="hasApproveOpinion">
...
import scoreEvaluateEdit from './components/scoreEvaluateEdit'
...
export default {
  ...
  methods: {
    ...
    // 填写审批意见
    async submitOpinion () {
      if (this.$refs.scoreEvaluate) {
        let isPass = await this.$refs.scoreEvaluate.addFinishScore()
        if (!isPass) return
      }
      this.isShowSubOpinion = true
    },
    // 填写审批意见1
    async submitOpinion1 () {
      if (this.$refs.scoreEvaluate) {
        let isPass = await this.$refs.scoreEvaluate.addFinishScore()
        if (!isPass) return
      }
    ...
    getProSubmitInfo (info) {
      ...
      this.processSubmitForm.businessSerialNumber = this.processParams.businessSerialNumber
      this.processSubmitForm.approveNo = this.processParams.businessSerialNumber
    ...
    abandonFlow () {
      this.$confirm(' ', '是否确认作废?', {
        ...
        .then(() => {
          API.deleteApprove(this.processParams.businessSerialNumber)
            .then(res => {
              this.$message({
                type: 'success',
                center: true,
    ...
    // 获取打分列表
    queryApproveFinishScore () {
      let param = {
        projectCode: this.projectCode || this.budgetData.projectCode,
        approveNo: this.processParams.businessSerialNumber
      }
      API.queryApproveFinishScore(param)
        .then(res => {
          if (res) {
