    ...
    <div class="top-head" id="top-head">
      <div class="head-con">
        <span class="group-btn" v-if="isEditable">
          ...
          <i class="approve-icon svgfont-btn-trace" v-if="this.$route.query.newBeforeLaunch==='false'" @click="processTrace"><span class="global-family">流程跟踪</span></i>
          <i class="approve-icon svgfont-btn-cancellation" v-if="this.$route.query.newBeforeLaunch==='false'" @click="cancellation"><span class="global-family">作废</span></i>
        </span>
          ...
          <el-form
            ...
            <el-form-item
              ...
              <el-input
                ...
                type="textarea"
                maxlength="300"
                v-model.trim="approveTimeForm.desc"
              ></el-input>
            </el-form-item>
          </el-form>
        </div>
...
export default {
  ...
  data () {
    return {
      ...
      businessFormUrl: '/groupPlan/beforeLaunchFlow/beforeLaunchFlowEdit', //首页我的草稿跳转URL
      ...
      isnewBeforeLaunch: this.$route.query.newBeforeLaunch,
      isEditable: false, //用户是否具有编辑权限
      projectApprove: {}, //表单信息
      loading: false,
      approveTimeForm: { // 预立项评审通过时间
        desc: null
      },
      approveTimeRule: {
        desc: [
          { required: true, message: '请输入预立项说明', trigger: ['change', 'blur'] }
        ]
      },
      docParams: { // 附件上传携带参数
        businessType: this.$route.query.businessType,
  ...
  created () {
    ...
    switch (this.projectCP) {
      case 'ug':
        this.launchTitle = '集团统筹项目-承担单位项目预立项申请（一般支撑）'
        break
      case 'us':
        this.launchTitle = '集团统筹项目-承担单位项目预立项申请（重大重点）'
        break
    }
    // 业务单据类型判断
    switch (this.$route.query.projectCategory) {
      case 'u':
        this.businessType = 'BT-UNDERTAKER-PRE-APPROVE'
        break
    }

    if (this.$route.query.newBeforeLaunch === 'true') {
      // 新增预立项单据时项目信息查询通过projectCode和versionNo查询获取
  ...
  methods: {
      ...
      API.queryApprove(params)
        .then(res => {
          if (res.projectApproveInfo && res.projectApproveInfo.extendedAttributes) {
            ...
            this.approveTimeForm.desc = extendsObj.desc
          }
          this.budgetData.versionNo = res.versionNo
    ...
    isShowEditBtn () {
      ...
      projectAPI.checkUserProjectPermission(params)
        ...
        .catch(e => {
          ...
          if (e.message) {
            this.$message({
              message: e.message || '预立项编辑按钮权限判断失败，请稍后重试！',
              type: 'error',
              showClose: true
            })
          }
    ...
    saveLaunchInfo (confirm = false) {
      ...
      if (this.approveTimeForm.desc !== null) {
        this.approveTimeForm.desc = this.approveTimeForm.desc
      }
      let urlString = '?newBeforeLaunch=false&businessType=' + this.businessType + '&projectCode=' + this.$route.query.projectCode + '&developPackage=' + this.$route.query.developPackage + '&projectCategory=' + this.$route.query.projectCategory
      let params = {
        businessType: this.businessType, // 业务类型
        projectCode: this.projectCode, // 项目编码
        attachmentIdList: this.attachmentIds, // 附件id集合
        extendedAttributes: JSON.stringify(this.approveTimeForm), // 单据表单预立项时间通过时间
        ...
        if (valid) {
          API.saveApprove(params)
            .then(res => {
              ...
              this.$router.push({
                query: merge(this.$route.query, { 'newBeforeLaunch': 'false', 'approveNo': this.approveNo })
              })
    ...
    processTrace () {
      ...
      if (this.$route.query.newBeforeLaunch === 'false' && this.approveNo) {
