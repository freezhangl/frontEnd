  ...
  <div class="main-content">
    <div class="head-btn" v-if="showLaunchBtn">
      <span style="float: right;">
        <el-button class="normal-btn" @click="addLaunchFlow">发起预立项流程</el-button>
      </span>
    </div>

    <div class="launch-info" v-if="showLaunchFlag">
      <project-panel title="预立项流程" style="border:1px solid #E5E5E5;">
    ...
    <div v-else style="margin-top:30px;text-align:center;color:#333333;">暂无预立项流程信息</div>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      projectCP: '',
      showLaunchBtn: false, //预立项流程发起按钮是否展示
      urlParam: '/groupPlan/beforeLaunchFlow/', // 计预立项流程单据新增/编辑/详情对应的url前缀
  ...
  methods: {
    ...
    // 判断发起预立项流程按钮是否展示
    isShowBeforeLaunchBtn () {
      const params = {
        projectCode: this.projectCode,
        businessType: this.businessType // 预立项单据业务类型编码
      }
      ...
      API.validateApprove(param)
        .then(res => {
          if (res === true) {
              ...
              .then(res => {
                if (res === true) {
                  this.$confirm(' ', '是否自动带入之前上传的附件？', {
                    ...
                    .then(() => {
                      this.$router.push({
                        path: this.urlParam + 'beforeLaunchFlowEdit',
                        query: {
                          newBeforeLaunch: 'true',
                          businessType: this.businessType, // 预立项单据业务类型编码
                          projectCode: this.projectCode,
            ...
            this.$message({
              message: '不能重复发起预立项申请或当前项目信息未填写完整，请检查！',
              type: 'warning',
              showClose: true
            })
          }
        ...
        .catch(e => {
          ...
          if (e.message) {
            this.$message({
              message: e.message || '发起预立项流程失败，请稍后重试！',
              type: 'error',
              showClose: true
            })
          }
      ...
      API.validateApprove(param)
        .then(res => {
          if (res === true) {
            ...
            this.$message({
              message: '不能编辑预立项申请或当前项目信息未填写完整，请检查！',
              type: 'warning',
              showClose: true
            })
          }
        ...
        .catch(e => {
          ...
          if (e.message) {
            this.$message({
              message: e.message || '编辑预立项流程失败，请稍后重试！',
              type: 'error',
              showClose: true
            })
          }
    ...
    openDetailPage () {
      // 需要携带参数
      this.$router.push({
        path: this.urlParam + 'beforeLaunchFlowDetail',
        query: {
          businessType: this.businessType, // 预立项单据业务类型编码
          projectCode: this.projectCode,
          approveNo: this.approveNo, //业务流水号
