<template>
  <div class="projectDocumentUpload" id="projectDocumentUpload">
    <project-panel class="upload-info" title="文档资料">
      <el-table
        ...
        <el-table-column
          ...
          fixed="right">
          <template slot-scope="scope">
            <div v-if="scope.row.fileName !== ''" style="display:flex">
...
export default {
  ...
  data () {
    return {
      signInfo: {}, // 文件sign信息
      tableHeadStyle: { // 文档资料表头样式
        background: '#F4F4F4',
        color: 'rgba(0,0,0,0.85)'
      },
      projectDocList: [], // 文档资料列表数据
      ...
      // uploadUrl: getStore().getters['CONFIG/baseUrl'] + '/zuul/rms/project/use/file/v1/4/fileUpload',
  ...
  methods: {
    ...
    getAttachmentList () {
      if (this.fileList) {
        ...
        this.$emit('existAttachmentIds', ids, this.fileList)
      } else {
        this.$emit('existAttachmentIds', [], [])
      }
    },
    // 日期格式化
    formatDate (val) {
    ...
    deleteAttachment (row, index) {
      this.$confirm(' ', '此操作将删除项目相关信息，是否进行此操作?', {
        ...
        .then(() => {
          ...
          this.$emit('existAttachmentIds', this.existAttachmentIds, this.projectDocList)
        })
    },
    // 文档个数限制--超出文件最大上传数
    ...
    handleSuccess (response, file) {
      if (this.currentUploadInfo.size + 1 > 15) {
        ...
        if (response.code === '000000') {
          ...
          let addFile = {
            businessType: this.currentUploadInfo.businessType,
            id: response.body.id,
            attachmentTypeCode: this.currentUploadInfo.attachmentTypeCode,
            fileType: this.currentUploadInfo.attachmentTypeCode,
            ...
            uploadDate: this.formatDate(response.body.modifyDate),
            span: 0,
            size: this.currentUploadInfo.size + 1,
            uploadRestrictions: this.currentUploadInfo.uploadRestrictions,
            creatorName: response.body.creatorName,
            createTime: response.body.modifyDate,
          ...
          this.$message({
            message: response.message || '文档资料上传失败!',
            type: 'error',
            showClose: true
          })
          this.uploadLoading.close()
    ...
    uploadCheck (row, index) {
      ...
      this.uploadAttachParams = {
        waterMarkFlag: '122放大',
        businessType: this.docParams.businessType, // 业务单据类型
        attachmentTypeCode: this.currentUploadInfo.attachmentTypeCode, // 附件类型
        projectCode: this.$route.query.projectCode || this.docParams.projectCode // 项目编码
      }
