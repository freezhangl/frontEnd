...
export default {
      ...
      flowAPI.getWorkItemDetail(parseInt(to.query.workItemID))
        .then(res => {
          if (res.currentState === 4 || res.currentState === 10) {
            ...
            next('/groupPlan/beforeLaunchFlow/beforeLaunchFlowDetail?' + urlString)
          }
        })
        .catch(e => {
      ...
      handler: function (val, oldVal) {
        ...
        switch (val) {
          case 'u':
            ...
            this.docParams.businessType = 'BT-UNDERTAKER-PRE-APPROVE'
            break
        }
      }
    }
  ...
  methods: {
    ...
    saveLaunchInfo (confirm = false) {
      ...
      let urlString = '?newBeforeLaunch=false&businessType=' + this.businessType + '&projectCode=' + this.projectCode + '&developPackage=' + this.developPackage + '&projectCategory=' + this.projectCategory
      let params = {
        approveNo: this.processParams.businessSerialNumber,
        ...
        businessFormUrl: '/beforeLaunchFlowEdit/beforeLaunchFlowEdit?addLaunch=false&businessType=01&' + urlString
      }
