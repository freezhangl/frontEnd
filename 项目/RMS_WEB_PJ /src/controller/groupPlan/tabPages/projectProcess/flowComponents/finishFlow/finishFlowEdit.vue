    ...
    <div class="top-head" id="top-head">
      <div class="head-con">
        <span class="group-btn" v-if="isEditable">
          ...
          <i class="approve-icon svgfont-btn-trace" v-if="this.$route.query.newFinish==='false'" @click="processTrace"><span class="global-family">流程跟踪</span></i>
          <i
            class="approve-icon svgfont-btn-cancellation"
            v-if="this.$route.query.newFinish==='false'"
            @click="cancellation"
        ...
        <div class="review-time">
          <el-form
            ...
            <el-form-item
              ...
              prop="planFinishTime">
              <el-date-picker
                v-model="approveTimeForm.planFinishTime"
                type="date"
...
export default {
  ...
  data () {
    var validOutLinePassTime = (rule, value, callback) => {
      ...
      /* if (this.projectBaseInfo.planApprovalTime && new Date(value)
        .getTime() < this.projectBaseInfo.planApprovalTime) {
        callback(new Error('结项评审通过时间不能早于计划立项时间'))
      } */
      callback()
    }
    return {
      ...
      businessFormUrl: '/groupPlan/finishFlow/finishFlowEdit', //首页我的草稿跳转URL
      ...
      outLinePassTimeOptions: {
        disabledDate: time => {
          /* if (this.projectBaseInfo.planApprovalTime) {
            return (
              time.getTime() < this.projectBaseInfo.planApprovalTime || time.getTime() > Date.now()
            )
          } */
          return (
  ...
  created () {
    ...
    if (this.$route.query.newFinish === 'true') {
  ...
  methods: {
      ...
      API.queryApprove(param)
        .then(res => {
          if (res.projectApproveInfo && res.projectApproveInfo.extendedAttributes) {
            ...
            this.approveTimeForm.planFinishTime = this.formatDate(extendsObj.planFinishTime)
            this.approveTimeForm.remark = extendsObj.remark
          }
          this.projectApprove = res.projectApproveInfo
          this.approveNo = res.projectApproveInfo.approveNo // 业务流水号
    ...
    saveFinalInfo (confirm) {
      ...
      this.$refs['approveTimeForm'].validate((valid) => {
        if (valid && validType) {
          if (this.approveTimeForm.planFinishTime !== null) {
            this.approveTimeForm.planFinishTime = new Date(this.approveTimeForm.planFinishTime)
              .getTime()
          }
          let urlString = '?newFinish=false&businessType=' + this.businessType + '&projectCode=' + this.$route.query.projectCode + '&developPackage=' + this.$route.query.developPackage + '&projectCategory=' + this.$route.query.projectCategory
          let params = {
          ...
          API.saveApprove(params)
            .then(res => {
              this.$router.push({
                query: merge(this.$route.query, { 'newFinish': 'false', 'approveNo': res.approveNo })
              })
    ...
    confirmNextHandle (val) {
      ...
      let params = {
        ...
        participantList: this.submitNextHandleInfo.participants //下一节点处理人
      }
    ...
    processTrace () {
      ...
      if (this.$route.query.newFinish === 'false' && this.approveNo) {
