      ...
      <div class="main-content">
        ...
        >
          ...
          <div class="wrapper_fin">
            <div class="wrapper_pin">请您对本委托函的完成情况进行评价</div>
            <table
              class="budget-con"
              border="1"
              cellpadding="0"
              cellspacing="0">
              <tr>
                <th class="col-title first">
                  <span>分数</span>
                </th>
                <th>评价</th>
              </tr>
              <tr >
                <td class="data-cell bg-title">
                  <el-form-item style="width:100%" prop="score">
                    <el-input
                ...
                <td  class="data-cell_pin bg-title">
                  <template>
                    <el-form-item style="width:100%;" prop="evaluate">
                      <el-input
                        :maxlength="300"
            ...
            <div style="margin-top:20px">
              <span style="margin-right: 30px;">打分标准：95-100:优秀</span><span> 90-94分:良好</span>
            </div>
            <div>
              <span style="margin:0 42px 0 70px"> 80-89:合格</span><span>80分以下：待改进</span>
            </div>
            <div style="display:flex">
              <div style="display:flex">
                <span style="width:75px">完成情况：</span>
              </div>
              <span style="margin-top:0;width:1100px;word-break:break-all;font-family: none;">{{ruleForm.completion}}</span>
            </div>
          </div>
        </el-form>
        <!-- 基本信息 -->
...
export default {
  ...
  methods: {
    ...
    inputHandler () {
      ...
      if (temp < 0) temp = '0'
      this.ruleForm.score = temp.replace(/[^\d.]/g, '')
    },
    setInputValue () {
      if (isNaN(this.ruleForm.score)) return '0'
      this.ruleForm.score = parseFloat(this.ruleForm.score) || '0'
    },
    // 获取流程指定节点按钮配置
    getFlowBtnList () {
      let params = {
      ...
      API.queryApprove(params)
        .then(res => {
          ...
          if (res.entrustLetterInfo) {
            ...
            if (res.entrustLetterInfo.entrustLetterEvaluate && res.entrustLetterInfo.entrustLetterEvaluate.evaluateScore) {
              this.ruleForm.score = res.entrustLetterInfo.entrustLetterEvaluate.evaluateScore
            }
            if (res.entrustLetterInfo.entrustLetterEvaluate && res.entrustLetterInfo.entrustLetterEvaluate.evaluateContent) {
              this.ruleForm.evaluate = res.entrustLetterInfo.entrustLetterEvaluate.evaluateContent
            }
          }
    ...
    saveEvaluate (confirm = false) {
      ...
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          API.saveEntrustLetterEvaluate(params)
            .then(res => {
              ...
              if (confirm) { //提交下一处理：保存成功后弹出下一步组件
                ...
                // 提示保存成功是否提交申请
                this.$confirm(' ', '评价打分保存成功，是否提交下一处理?', {
