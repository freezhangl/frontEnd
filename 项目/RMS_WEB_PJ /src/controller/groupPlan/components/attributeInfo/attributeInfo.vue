<!-- 新增组件 JSON.stringify(projectAttribute) == '{}' && !$route.params.projectCode -->
<template>
...
export default {
          ...
          blockDtos.forEach((item, index) => {
            item.fieldList.forEach((item1, index1) => {
              ...
              if (item1.fieldKey === 'accountingSubject') { //核算主体 只有草稿状态可编辑
                this.$refs.formProjectAdd.list.blockDtos[index].fieldList[index1].isReadonly = (this.receiveData.projectDetailsStatus && this.receiveData.projectDetailsStatus === '0') ? '0' : '1'
              }
            })
          })
          // IC_A - 应用基础研究
  ...
  mounted () {
    ...
    // this.obj = {
    //   parameters: 'id=1290211824813215745,projectCode=R201023E,projectName= 新增项目zch测试属性信息001,projectSource=PS-COMPANY-DEVELOP,projectClass=1,projectCategory=PC-SELF-UNIT,developProperty=DP-PRODUCT-DEVELOP,projectType=PT-DI,projectLevel=PL-KEY-PJ,planApprovalTime=1597075200000,planFinishTime=1598457600000,actualApprovalTime=,actualFinishTime=,projectExpenditureType=PET-OPEX,periodOwnership=1,projectYear=2020,projectDetailsStatus=0,developPackage=1,businessFormUrl=/projectInfo/projectInfo-edit/R201023E',
    //   companyCode: 'cmss',
