    ...
    <project-panel title="资源信息">
      <div class="res-info-body">
        <div class="select">
          <el-row>
            <el-col :span="12">
              是否需要EP资源：<span :class="{'highlightStyle':receiveData&&receiveData.highlightProps && receiveData.highlightProps.needEpResource}">{{needEpResource=='1' ? '是' : '否'}}</span>
            </el-col>
            <el-col :span="12">
              是否交维：<span :class="{'highlightStyle':receiveData&&receiveData.highlightProps && receiveData.highlightProps.deliveryMaintenance}">{{deliveryMaintenance=='1' ? '是' : '否'}}</span>
            </el-col>
          </el-row>
        </div>
