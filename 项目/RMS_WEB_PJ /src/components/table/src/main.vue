        ...
        >
          <template slot-scope="scope">
            ...
            <span v-if="item.type === 'dynamicButton'">
              <template v-for="btn in item.btnList">
                <el-button
                  v-if="btn.whenShow.indexOf(scope.row[item.impactProp])>-1 && !btn.dropDownList"
                  :key="btn.id"
                  :disabled="btn.isDisabled && btn.isDisabled(scope.row)"
                  :type="btn.type"
                  :size="btn.size || size"
                ...
                <el-dropdown
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <template v-for="ele in btn.dropDownList">
                      <el-dropdown-item
                        v-if="ele.whenShow.indexOf(scope.row[item.impactProp])>-1"
            ...
            <span
              ...
              :class="item.itemClass && item.itemClass(scope.row,item.prop)"
...
export default {
  ...
  props: {
    ...
    indexWidth: { type: String, default: '80px' },
    // 是否显示分页
    ...
    //为 Table 中的某一行添加 class，表明该行处于某种状态{row, rowIndex}
    rowClassName: { type: Function, default: null },
    //为单元格添加className， Function({row, column, rowIndex, columnIndex})/String
