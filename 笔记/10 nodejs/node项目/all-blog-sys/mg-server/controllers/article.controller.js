...
exports.findAllGroup = (req, res) => {
    ...
    //按月统计
    let sql = "SELECT DATE_FORMAT( createdAt, \"%Y-%m\" ) AS mouth , " +
        " GROUP_CONCAT(`article_title`) AS list, " +
        " GROUP_CONCAT(`id`) AS idList, " +
        " GROUP_CONCAT(`createdAt`) AS timeList, " +
        "count(id) AS count\n" +
        "FROM mg_articles \n" +
        "GROUP BY DATE_FORMAT( createdAt, \"%Y-%m\" )  ORDER BY createdAt DESC"
    //按天数
    let daySql = " SELECT DATE_FORMAT( createdAt, '%Y-%m-%d' ) AS day ," +
        "count(id) AS count FROM mg_articles " +
        "GROUP BY DATE_FORMAT( createdAt, '%Y-%m-%d' )  ORDER BY day DESC"
    ...
    DAO.doQuery(daySql, dayData => {
        DAO.doQuery(sql, monthData => {
            ...
            r.data.dayData = dayDatasNum
            r.data.monthData = monthData.data[0]
            res.sendResult(r)
        })
